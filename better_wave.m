...
nonlinear = 0; %%% Set true to include nonlinear terms, i.e. du/dt + u du/dx + g dz/dx = 0 and dz/dt + d/dx ( (H+z) u ) = 0

godunov = 1; %%% Set true to use godunov method (shock-resolving) 

leapfrog = true; %%% Set true to offset u and zeta in time, increasing the accuracy of the time-stepping to second order
...
for i = 1:nc
    ...
    zeta(1,i) = 15*exp(-((i-nc/2)/(nc/10))^2);
    %zeta(1,i) = 15 * sin(pi*i/(nc));
    %zeta(1,i) = 20*exp(-i^2/200)+sin(3*i/nc);
end
